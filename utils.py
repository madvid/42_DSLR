...
def print_predict_usage():
	...
	str_usage += "  python logreg_predict.py --dataset=... --model=...\n"
	str_usage += BLUE + "Args:\n" + END
...
def print_train_usage():
	...
	str_usage += YELLOW + "  --method=[gradient-descent/stochastic-gradient-descent/stochastic-gradient-descent+momentum/minibatch]:\n" + END
...
# Functions to display the usages                                             #
# --------------------------------------------------------------------------- #
def open_read_file(filename:str, context:str="predict"):
	...
	if context == "train":
		if not all([val in ["Gryffindor", "Slytherin", "Hufflepuff", "Ravenclaw"] for val in df["Hogwarts House"].unique()]):
			print(RED + "Column Hogwarts House seems to be empty." + END)
			return None
